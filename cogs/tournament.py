...
async def _announce_state_change(
    ...
    if kind == "started":
        ...
        body = (
            f"{t['match_format']} · Swiss · rank-weighted pairings incoming.\n"
            "Round 1 details drop in the next announcement."
        )
    else:
...
class Tournament(commands.Cog):
    ...
    async def tournament_start(
        ...
        await _announce_state_change(self.bot, t_after, "started")

        await interaction.followup.send(
            f"🔔 **{name}** is now in progress with **{count}** players. "
            "Round-1 pairing and match channels arrive in the next slice — "
            "for now the signup panel is locked and entrants have been pinged.",
            ephemeral=True,
        )
